...
| **Fleet platforms**   | `True`            | simulated processes that subscribe to TaskAssignments and publish their own position | military-style: `UUV-Alpha`, `USV-Bravo` | query + task    |
...
00:15  "Task UUV-Alpha to 56.15 north, 15.58 east." → readback → "Confirm." → dispatch
00:30  Platform visibly moves on map.
00:40  "Task USV-Bravo to 56.18 north, 15.64 east." → readback → "Confirm." → dispatch
01:00  "Recall UUV-Alpha to base."                  → readback → "Confirm." → dispatch
...
- [src/mock_world_model.py](src/mock_world_model.py) — in-memory stand-in for the world model HTTP API. `task_waypoint` produces a NATO-phonetic readback; `recall_to_base` produces a crisp `"<call sign>, recall to base. Confirm."` and stages with `BASE_LATITUDE=56.16`, `BASE_LONGITUDE=15.59` (Karlskrona harbor; matches b-service operator default). `Platform` carries `is_controllable`; fleet (`UUV-Alpha`, `UUV-Bravo`, `UUV-Charlie`, `USV-Delta`, `USV-Echo`) is `True`, ambient seed (`MV Northern Star`, `FV Karlsvik`, `MV Stumholmen`) is `False` — Stumholmen sits at the captured PCAP coordinate and is stationary. Both staging paths refuse ambient with `UNKNOWN_CALLSIGN`. Resolver normalises hyphens/spaces so STT's "UUV Alpha" matches canonical "UUV-Alpha".
...
- [ ] **6. Map UI** — Leaflet + WS client subscribing to b-service `WS /api/v1/ws`. Ambient grey, controllable blue. Audit-log panel. **Mic on/off control** for the LiveKit room.
