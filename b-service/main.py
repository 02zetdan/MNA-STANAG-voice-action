...
async def ingest_endpoint(payload: IngestPayload):
    ...
    is_first_sighting = payload.track_id not in engine.tracks
    ...
    if is_first_sighting and payload.lat is not None and payload.lon is not None:
        audit.append({
            "kind": "ingest",
            "message": f"first contact: {payload.track_id}",
            "details": {"track_id": payload.track_id, "lat": payload.lat, "lon": payload.lon,
                        "source": payload.source, "is_controllable": payload.is_controllable},
        })
